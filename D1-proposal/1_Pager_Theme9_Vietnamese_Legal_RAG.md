...
**Theme**: Theme 9 — Agentic RAG | **Nhóm thực hiện**: 4 thành viên (Lead & Orchestrator, Graph Retrieval Engineer, Web Search & Verification Engineer, Security & Evaluation)
...
1. **Hệ thống Đa tác tử (Multi-Agent RAG)**: Phân vai chuyên biệt dưới *Orchestrator*:
   ...
   - *Claim Auditor Agent*: Soát lại từng câu so với văn bản gốc theo mô hình FActScore để loại bỏ hoàn toàn việc bịa điều luật [4], [7].
...
3. **Tự kiểm tra & Soát lỗi (Reflection & Verification)**: Vòng lặp giữa Drafter và Auditor (Self-RAG [11] & Reflexion [12]) bóc tách và đối soát từng câu so với điều luật gốc, đảm bảo 100% dẫn chứng có căn cứ từ văn bản còn hiệu lực.
4. **Bảo mật & Cổng phê duyệt an toàn (Security & Guardrails)**: Phân quyền công cụ MCP 3 cấp độ. Thao tác xuất báo cáo hoặc xác nhận tuân thủ bắt buộc phải có sự phê duyệt của người dùng (Human-in-the-loop). Làm sạch tài liệu đầu vào để chặn Prompt Injection [9].
...
- **[9]** K. Greshake et al., "Not What You've Signed Up For: Compromising Real-World LLM-Integrated Applications with Indirect Prompt Injection," *ACM AISEC 2023*.
- **[11]** A. Asai et al., "Self-RAG: Learning to Retrieve, Generate, and Critique through Self-Reflection," *ICLR 2024*.
